...
const Triangle = (x1, y1, x2, y2, x3, y3) => {
  const perimeter = () => {
    const lenght1 = Math.sqrt(Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2)); // длины сторон треугольника
    const lenght2 = Math.sqrt(Math.pow(x3 - x2, 2) + Math.pow(y3 - y2, 2));
    const lenght3 = Math.sqrt(Math.pow(x3 - x1, 2) + Math.pow(y3 - y1, 2));
  ...
  const medians = () => {
    const Mx1 = (x2 + y3) / 2; // медиана из точки x 1-я координа
    const Mx2 = (y2 + y3) / 2; // медиана из точки x 2-я коорд
    const My1 = (x1 + x3) / 2; // медиана из точки y 1-я коорд
    const My2 = (y1 + y3) / 2; // медиана из точки y 2-я коорд
    const Mz1 = (x1 + x2) / 2; // медиана из точки z 1-я коорд
    const Mz2 = (y1 + y2) / 2; // медиана из точки z 2-я коорд
    console.log(`координаты пересечения медианы, вышедшей из точки x1y1: (${Mx1};${Mx2})`);
    console.log(`координаты пересечения медианы, вышедшей из точки x2y2: (${My1};${My2})`);
    console.log(`координаты пересечения медианы, вышедшей из точки x3y3: (${Mz1};${Mz2})`);
  };
  const getInfo = () => {
    console.log(`координаты треугольника: A(${x1},${y1}) B(${x2},${y2}) C(${x3},${y3})`);
  };
  return { x1, y1, x2, y2, x3, y3, perimeter, square, medians, getInfo };
};

const Triangle1 = Triangle(0, 0, 1, 1, 1, 0);
Triangle1.perimeter();
Triangle1.square();
Triangle1.medians();
Triangle1.getInfo();
