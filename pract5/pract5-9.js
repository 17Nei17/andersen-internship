...
class Restangle extends Figure {
  ...
  getPerimeter(){
    let lenght1 = Math.sqrt(Math.pow(this.x2-this.x1,2)+Math.pow(this.y2-this.y1,2)); //длины сторон прямоугольника
    let lenght2 = Math.sqrt(Math.pow(this.x3-this.x2,2)+Math.pow(this.y3-this.y2,2)); 
    let P = (lenght1+lenght2)*2;
    return P;
  }
  getRestangle (){
    console.log (`Текущие значения координат прямоугольника: сторона X1(${this.x1},${this.y1}) сторона X2(${this.x2},${this.y2}) cторона X3(${this.x3},${this.y3}) сторона X4 (${this.x4},${this.y4})`);
  }
}
...
class Triangle extends Figure{
  ...
  getTriangle(){
    console.log (`Текущие значения координат прямоугольника: сторона X1(${this.x1},${this.y1}) сторона X2(${this.x2},${this.y2}) cторона X3(${this.x3},${this.y3})`)
  }
}

let Restangle1 = new Restangle(1,1,2,2,3,3,4,4);
